...
const MyPosts = React.memo(props => {
  let Posts = props.PostsData.map(PostItem => (
    <Post
      key={PostItem.id}
      id={PostItem.id}
      name={PostItem.name}
      message={PostItem.message}
      likeCounter={PostItem.likeCounter}
      time={PostItem.time}
    />
  ));
...
const AddPostForm = props => {
  ...
  return (
    <Formik
      ...
      validate={values => {
        const errors = {};
        if (values.NewPostText.length >= 300) {
          errors.NewPostText = 'Your post is very long';
        }
        return errors;
      }}
    >
